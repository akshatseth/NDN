...
class Vehicle:
    def __init__(self, vehicle_type, router_host, router_port,backup_router_host,backup_router_port,port):
        ...
        self.gear_sensor = bikeGear('%s/gear' % self.vehicle_type)
        self.petrol_sensor = bikePetrol('%s/petrol' % self.vehicle_type)
        self.advertise_string = '[%s/location, %s/speed, %s/direction, %s/temperature, \
            %s/status, %s/pressure, %s/speedup, %s/proximity, %s/gear, %s/petrol]' % (self.vehicle_type, self.vehicle_type, self.vehicle_type, self.vehicle_type,
            self.vehicle_type, self.vehicle_type, self.vehicle_type, self.vehicle_type, self.vehicle_type, self.vehicle_type)
...
def main():
    ...
    bike = Vehicle('bike', router_host, router_port,backup_router_host,backup_router_port, peer_port)
    t1 = threading.Thread(target=bike.advertiseFeature)
    t1.start()
